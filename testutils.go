...
func areCollectionsEqual(t *testing.T, c1, c2 *Collection) {
	...
	assert.Equal(t, c1.rootPgNum, c2.rootPgNum)

}

func areTreesEqual(t *testing.T, t1, t2 *Collection) {
	t1Root, err := t1.tx.getNode(t1.rootPgNum)
	require.NoError(t, err)

	t2Root, err := t2.tx.getNode(t2.rootPgNum)
	require.NoError(t, err)
...
func createTestMockTree(t *testing.T) (*Collection, func()) {
	...
	expectedCollection, err := tx.CreateCollection(testCollectionName)
	expectedCollection.rootPgNum = root.pageNum
	require.NoError(t, err)
